...
Connect-AzAccount
New-AzResourceGroup -Name $MonitoringHackName -Location 'West Europe' #The hack uses a few features in preview so its best to leave this in East US
$rg = get-Azresourcegroup -Name $MonitoringHackName
...
New-AzKeyVault -VaultName $MonitoringHackVaultName -ResourceGroupName $rg.ResourceGroupName -Location $rg.Location -EnabledForTemplateDeployment
...
Set-AzKeyVaultSecret -VaultName $MonitoringHackVaultName -Name "VMPassword" -SecretValue (Get-Credential).Password
...
(Get-AzKeyVault -VaultName $MonitoringHackVaultName).ResourceId
...
New-AzResourceGroupDeployment -Name $MonitoringHackName -ResourceGroupName $MonitoringHackName -TemplateFile '.\VMSSazuredeploy.json' -TemplateParameterFile '.\azuredeploy.parameters.json'